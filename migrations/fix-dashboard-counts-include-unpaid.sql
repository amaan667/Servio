-- Fix dashboard_counts to include ALL orders, not just paid ones
-- Orders should count regardless of payment status
...
BEGIN
    ...
    -- Count live orders (today within live window) - ALL orders, exclude only CANCELLED
    ...
    -- Count earlier today orders (today but before live window) - ALL orders, exclude only CANCELLED
    ...
    -- Count history orders (before today) - ALL orders, exclude only CANCELLED
    ...
    -- Count total today's orders - ALL orders, exclude only CANCELLED
    ...
    -- Count active tables (tables with current orders) - only active statuses
