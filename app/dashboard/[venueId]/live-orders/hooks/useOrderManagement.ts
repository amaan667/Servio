...
export function useOrderManagement(venueId: string) {
  ...
  useEffect(() => {
    const loadOrders = async () => {
      ...
      if (!allError && allData) {
        const liveOrderIds = new Set((liveData || []).map((order: Record<string, unknown>) => order.id));
        const allTodayFiltered = allData.filter((order: Record<string, unknown>) => !liveOrderIds.has(order.id));
