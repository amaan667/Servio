...
async function clearTableSession(orderData: unknown, venueId: string, orderId: string) {
  try {
    ...
    if (orderData.table_id) {
      filteredActiveOrders = (activeOrders || []).filter((o: unknown) => o.table_id === orderData.table_id);
    } else if (orderData.table_number) {
      filteredActiveOrders = (activeOrders || []).filter((o: unknown) => o.table_number === orderData.table_number);
    }
