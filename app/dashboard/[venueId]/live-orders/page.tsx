...
export default async function LiveOrdersPage({
  ...
  try {
    ...
    if (!hasAuthCookie) {
      console.log('[LIVE-ORDERS] No auth cookie found, redirecting to sign-in');
      redirect('/sign-in');
    }
    ...
    log('LIVE-ORDERS SSR user', { hasUser: !!user, error: userError?.message });
    
    if (userError) {
      console.error('[LIVE-ORDERS] Auth error:', userError);
      redirect('/sign-in');
    }
    ...
    if (!venue) {
      console.log('[LIVE-ORDERS] Venue not found or user does not own it');
      redirect('/dashboard');
    }
    ...
    console.error('[LIVE-ORDERS] Unexpected error:', error);
    redirect('/sign-in');
  }
}