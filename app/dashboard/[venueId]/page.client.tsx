...
export default function VenueDashboardClient({ 
    ...
    if (todayWindow) {
      const checkDayChange = async () => {
        ...
        if (currentDay !== lastDay) {
          logInfo('[DASHBOARD] Day changed, resetting stats cache and clearing tables');
          ...
          if (venue) {
            try {
              logInfo('[DASHBOARD] Clearing all tables and sessions for new day');
              ...
              if (response.ok) {
                logInfo('[DASHBOARD] All tables and sessions cleared successfully');
              } else {
                logError('[DASHBOARD] Failed to clear all tables and sessions:', response.status);
              }
            } catch (error) {
              logError('[DASHBOARD] Error clearing all tables and sessions:', error);
            }
  ...
  useEffect(() => {
    if (!venue || !todayWindow) {
      logInfo('[DASHBOARD] Skipping subscription setup - venue or todayWindow not ready');
      return;
    }

    logInfo('[DASHBOARD] Setting up real-time subscription with window:', todayWindow);
    ...
    const channel = createClient()
      ...
      .on('postgres_changes', 
        ...
        async (payload: any) => {
          ...
          if (isInTodayWindow) {
            logInfo('[DASHBOARD] Order is within today window, updating counts and revenue');
            ...
            if (payload.event === 'INSERT' && payload.new) {
              logInfo('[DASHBOARD] New order inserted, updating revenue incrementally');
              ...
              logInfo('[DASHBOARD] Order updated, checking if revenue needs recalculation');
              ...
              if (payload.new.order_status === 'CANCELLED' || payload.new.order_status === 'REFUNDED') {
                logInfo('[DASHBOARD] Order cancelled/refunded, recalculating total revenue');
                await loadStats(venue.venue_id, todayWindow);
              }
            }
          } else {
            logInfo('[DASHBOARD] Order is outside today window, skipping updates');
          }
        }
      )
      .subscribe();
    ...
    const handleOrderCreated = (event: CustomEvent) => {
      logInfo('[DASHBOARD] Custom order created event received:', event.detail);
    ...
    return () => {
      logInfo('[DASHBOARD] Cleaning up real-time subscription and event listeners');
  ...
  const loadStats = async (vId: string, window: any) => {
    ...
    try {
      ...
      logInfo('[DASHBOARD] Found orders for today:', orders?.length || 0);
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          {venue?.has_tables !== false && (
            <Link 
              ...
              onClick={() => {
                logInfo('[DASHBOARD] Table Management clicked:', {
                  venueId,
