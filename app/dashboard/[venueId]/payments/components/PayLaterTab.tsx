...
interface PayLaterTabProps {
  orders: PaymentOrder[];
  onMarkPaid: (orderId: string) => Promise<void>;
  isProcessing: string | null;
}

export function PayLaterTab({ orders, onMarkPaid, isProcessing }: PayLaterTabProps) {
  ...
  if (payLaterOrders.length === 0) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground">No pay later orders</p>
      </div>
    );
  }
