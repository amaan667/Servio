...
export function useTableManagementState(venueId: string) {
  ...
  const { data: counterOrderCounts = { total: 0, pending: 0, in_prep: 0, ready: 0 } } = useCounterOrderCounts(venueId);
  useCounterOrdersRealtime(venueId);

  // Table orders
  const { data: tableOrders = [] } = useTableOrders(venueId);
  const { data: tableOrderCounts = { total: 0, pending: 0, in_prep: 0, ready: 0 } } = useTableOrderCounts(venueId);
