...
export default function StaffClient({
  ...
  useEffect(() => {
    const loadStaff = async () => {
      ...
      try {
        logInfo('[AUTH DEBUG] Loading staff for venue:', venueId);
        ...
        if (res.ok && !j?.error) {
          ...
          logError('[AUTH DEBUG] Failed to load staff:', j?.error);
        }
      } catch (e) {
        logError('[AUTH DEBUG] Failed to load staff:', e);
      }
    };
  ...
  useEffect(() => {
    const loadShifts = async () => {
      logInfo('[AUTH DEBUG] Loading shifts for venue:', venueId);
      ...
      if (res.ok && !j?.error) {
        ...
        logError('[AUTH DEBUG] Failed to load shifts:', j?.error);
  ...
  const staffCounts = useMemo(() => {
    logInfo('[STAFF DEBUG] initialCounts:', initialCounts);
    logInfo('[STAFF DEBUG] initialStaff:', initialStaff);
    logInfo('[STAFF DEBUG] staff:', staff);
    logInfo('[STAFF DEBUG] allShifts:', allShifts);
    ...
    if (hasRealTimeData) {
      logInfo('[STAFF DEBUG] Using real-time staff data');
    ...
    if (initialCounts) {
      logInfo('[STAFF DEBUG] Using initialCounts from server');
      return {
    ...
    logInfo('[STAFF DEBUG] No data available, returning zeros');
    return {
      totalStaff: 0,
      activeStaff: 0,
      uniqueRoles: 0,
