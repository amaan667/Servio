...
import { getValidatedSession, supabase, signOutUser, type AuthSession } from "@/lib/supabase"
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    logger.info("DASHBOARD: Session validated successfully", { userId: validatedSession.user.id, venueId: validatedSession.venue.venue_id })
  ...
  const fetchDashboardData = async (venueId: string) => {
    ...
    try {
      ...
      if (!error && orders) {
        ...
        const activeTables = new Set(orders.filter((o) => o.status !== "completed").map((o) => o.table_number)).size
        ...
        logger.info("DASHBOARD", { message: "Dashboard data fetched", stats, recentOrders })
      }
    } catch (error) {
      logger.error("DASHBOARD", { message: "Unexpected error fetching dashboard data", error })
    }
  }
  ...
  async function handleDeleteAccount() {
    ...
    try {
      const res = await fetch('/api/delete-account', {
        ...
        body: JSON.stringify({ userId: session.user.id, venueId: session.venue.venue_id }),
      })
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <Card className="hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardDescription>Update menu items, venue settings, and staff management</CardDescription>
            </CardHeader>
            <CardContent>
              <Button
                onClick={() => router.push(`/dashboard/${session.venue.venue_id}`)}
          ...
          <Card className="hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardDescription>See how customers view and place orders from your menu</CardDescription>
            </CardHeader>
            <CardContent>
              <Button
                variant="outline"
                onClick={() => window.open(`/order?venue=${session.venue.venue_id}&table=1`, "_blank")}
                className="w-full"
              >
          ...
          <Card className="hover:shadow-lg transition-shadow">
            <CardHeader>
              ...
              <CardDescription>Create QR codes for tables and print them for customer use</CardDescription>
            </CardHeader>
            <CardContent>
              <Button variant="outline" onClick={() => router.push("/generate-qr")} className="w-full">
          ...
          <CardContent>
            {recentOrders.length === 0 ? (
              <div className="text-center py-8">
                <p className="text-gray-600">No orders yet today. Orders will appear here as they come in.</p>
              </div>
            ) : (
