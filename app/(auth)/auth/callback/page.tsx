...
function OAuthCallbackContent() {
  ...
  useEffect(() => {
    ...
    // Increased timeout to 90 seconds for better reliability in production
    const timeout = setTimeout(() => {
      if (!finished) {
        console.log('[AUTH DEBUG] OAuth callback timeout after 90 seconds');
        setDebugInfo("Timeout after 90 seconds");
    ...
    (async () => {
      try {
        ...
        // 1) Exchange PKCE code on the **client** with retry logic
        const exchangeStartTime = Date.now();
        let exchangeAttempts = 0;
        const maxExchangeAttempts = 3;
        let exchangeError = null;
        
        while (exchangeAttempts < maxExchangeAttempts) {
          try {
            exchangeAttempts++;
            console.log('[AUTH DEBUG] 🔄 Exchange attempt', exchangeAttempts, 'of', maxExchangeAttempts);
            ...
            if (error) {
              exchangeError = error;
              console.log('[AUTH DEBUG] ❌ Exchange failed (attempt', exchangeAttempts, '):', {
                message: error.message,
                status: error.status,
                exchangeTime
              });
              
              // If it's a network error and we haven't exhausted attempts, retry
              if (exchangeAttempts < maxExchangeAttempts && (
                error.message.includes('network') || 
                error.message.includes('timeout') ||
                error.message.includes('fetch') ||
                error.message.includes('Failed to fetch')
              )) {
                console.log('[AUTH DEBUG] 🔄 Retrying exchange due to network error');
                setDebugInfo(`Retrying exchange (${exchangeAttempts}/${maxExchangeAttempts})...`);
                await new Promise(resolve => setTimeout(resolve, 2000 * exchangeAttempts)); // Exponential backoff
                continue;
              }
            ...
            exchangeError = retryError;
            console.log('[AUTH DEBUG] ❌ EXCEPTION: Exchange error (attempt', exchangeAttempts, '):', {
              message: retryError?.message,
              stack: retryError?.stack
            });
            
            if (exchangeAttempts < maxExchangeAttempts) {
              console.log('[AUTH DEBUG] 🔄 Retrying exchange due to exception');
              setDebugInfo(`Retrying exchange (${exchangeAttempts}/${maxExchangeAttempts})...`);
              await new Promise(resolve => setTimeout(resolve, 2000 * exchangeAttempts)); // Exponential backoff
              continue;
            }
        ...
        if (exchangeAttempts >= maxExchangeAttempts && exchangeError) {
          console.log('[AUTH DEBUG] ❌ All exchange attempts failed');
          setDebugInfo("All exchange attempts failed");
          return router.replace(`/sign-in?error=exchange_failed&message=${encodeURIComponent(exchangeError.message || 'Exchange failed after multiple attempts')}`);
        }
