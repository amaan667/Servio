...
=======
      console.log('[AUTH DEBUG] Code exchange successful, checking session');
      const { data: sessionData } = await sb.auth.getSession();
      ...
      if (sessionData.session?.user) {
        console.log('[AUTH DEBUG] User authenticated, redirecting to:', next);
        router.replace(next);
      } else {
        console.log('[AUTH DEBUG] No session after code exchange, redirecting to sign-in');
        router.replace("/sign-in?error=no_session");
      }
    })().finally(() => {
      finished = true;
      clearTimeout(timeout);
      console.log('[AUTH DEBUG] Callback processing finished');
    });
>>>>>>> Stashed changes
  }, [router, sp]);
