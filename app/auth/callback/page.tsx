...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    addDebugLog(
      `[AUTH CALLBACK] User Agent: ${typeof window !== "undefined" ? navigator.userAgent : "SSR"}`
    );
    addDebugLog(
      `[AUTH CALLBACK] Current URL: ${typeof window !== "undefined" ? window.location.href : "SSR"}`
    );
    ...
    const handleCallback = async () => {
      try {
        ...
        addDebugLog(
          `[AUTH CALLBACK] URL params: ${JSON.stringify({
        ...
        if (state && !code) {
          addDebugLog(
            "[AUTH CALLBACK] State parameter present but no code - likely OAuth state mismatch"
          );
        ...
        if (!code) {
          ...
          addDebugLog(
            `[AUTH CALLBACK] All search params: ${JSON.stringify(
              searchParams
        ...
        addDebugLog(
          `[AUTH CALLBACK] Session check result: ${JSON.stringify({
        ...
        if (existingSession) {
          ...
          addDebugLog(
            `[AUTH CALLBACK] Redirecting to dashboard for venue: ${primaryVenue.venue_id}`
          );
        ...
        addDebugLog(
          `[AUTH CALLBACK] Exchange completed: ${JSON.stringify({
            hasData: !!data,
        ...
        if (exchangeError) {
          addDebugLog(
            `[AUTH CALLBACK] Exchange error details: ${JSON.stringify({
          ...
          try {
            ...
            addDebugLog(
              `[AUTH CALLBACK] Retry result: ${JSON.stringify({
                hasData: !!retryData,
            ...
            if (retryData?.session) {
              addDebugLog(
                "[AUTH CALLBACK] Fallback authentication successful, fetching primary venue..."
              );
        ...
        if (data?.session) {
          ...
          addDebugLog(
            `[AUTH CALLBACK] Session details: ${JSON.stringify({
        ...
        addDebugLog(
          `[AUTH CALLBACK] Error details: ${JSON.stringify({
