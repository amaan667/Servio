  ...
  useEffect(() => {
    ...
    const boot = async () => {
      try {
        ...
        if (!session?.user) {
          console.log('[COMPLETE-PROFILE] no session/user -> /sign-in');
          router.replace('/sign-in');
          return;
        }
        
        if (!cancelled) {
          console.log('[COMPLETE-PROFILE] setting user:', session.user.id);
    ...
    const timeout = setTimeout(() => {
      if (!cancelled) {
        console.warn('[COMPLETE-PROFILE] timeout fallback - showing form anyway');
        setUser({ id: 'timeout-fallback' }); // Minimal user object to show form
      }
    }, 3000);
