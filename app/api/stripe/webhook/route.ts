...
export async function POST(req: NextRequest) {
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Stripe signature found:',
    sig.substring(0, 20) + '...'
  );
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Raw payload preview:',
    raw.substring(0, 200)
  );
  ...
  try {
    ...
    logger.error(
      '[STRIPE WEBHOOK DEBUG] Webhook construction error:',
      getErrorMessage(err)
    );
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Processing checkout session:',
    session.id
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Session metadata:',
    JSON.stringify(session.metadata, null, 2)
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Original order ID from metadata:',
    originalOrderId
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Session payment status:',
    session.payment_status
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Session customer details:',
    session.customer_details
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Session amount total:',
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Looking for order with ID:',
    originalOrderId
  );
  ...
  const { data: originalOrder, error: fetchError } = await supabaseAdmin
    .from('orders')
    .select(
      'id, payment_status, customer_name, table_number, created_at, venue_id, order_status'
    )
  ...
  if (fetchError || !originalOrder) {
    ...
    logger.error(
      '[STRIPE WEBHOOK DEBUG] Original order not found:',
      originalOrderId
    );
    ...
    if (recentOrders && recentOrders.length > 0) {
      ...
      logger.debug(
        '[WEBHOOK] Successfully updated fallback order',
        fallbackOrder.id
      );
      return NextResponse.json({
        ok: true,
    ...
    return NextResponse.json(
      {
        ok: false,
        error: 'Original order not found and no fallback available',
      },
      { status: 404 }
    );
  }
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] ===== ORDER FOUND - PROCEEDING WITH UPDATE ====='
  );
  ...
  if (updateErr) {
    ...
    logger.error(
      '[STRIPE WEBHOOK DEBUG] Error details:',
  ...
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Successfully updated order',
    originalOrderId,
    'with payment status PAID'
  );
  logger.debug(
    '[STRIPE WEBHOOK DEBUG] Final response: { ok: true, orderId:',
    originalOrderId,
    '}'
  );
