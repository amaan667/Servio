...
export async function POST(req: Request) {
  try {
    const { amount, customerEmail, customerName, venueName, metadata } = await req.json();
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `Table ${metadata?.tableNumber || 'N/A'}`,
            },
      ...
      metadata: {
        ...
        orderType: 'customer_order', // Flag to distinguish from subscription
      },
    });
