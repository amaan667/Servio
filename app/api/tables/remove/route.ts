...
export async function POST(req: NextRequest) {
  try {
    ...
    if (tableError) {
      logger.error('[TABLE REMOVAL] Error removing tables:', { error: tableError instanceof Error ? tableError.message : 'Unknown error' });
      return NextResponse.json({ 
        ok: false, 
    ...
    logger.error('[TABLE REMOVAL] Unexpected error:', { error: error instanceof Error ? getErrorMessage(error) : 'Unknown error' });
    return NextResponse.json({ 
      ok: false, 
