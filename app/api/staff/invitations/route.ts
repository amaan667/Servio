...
export async function GET(_request: NextRequest) {
  try {
    ...
    const errorMessage = _error instanceof Error ? _error.message : "Internal server _error";
  ...
  try {
    ...
    if (venueOwner?.owner_user_id) {
      ...
      if (!ownerUserError && ownerUser?.email?.toLowerCase() === email.toLowerCase()) {
        return NextResponse.json(
          {
            error:
              "This is the owner's email address. The owner already has full access to this venue.",
          },
          { status: 409 }
        );
      }
    ...
    return NextResponse.json({
      ...
      invitationLink: emailSent
        ? undefined
        : (await import("@/lib/email")).generateInvitationLink(invitation.token),
    });
  } catch (_error) {
    const errorMessage = _error instanceof Error ? _error.message : "Internal server _error";
    return NextResponse.json(
...
export async function DELETE(_request: NextRequest) {
  try {
    ...
    const { error: deleteError } = await supabase.from("staff_invitations").delete().eq("id", id);
