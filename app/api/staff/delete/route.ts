...
export const POST = withUnifiedAuth(
  async (req: NextRequest, context) => {
    ...
    try {
      ...
      if (error) {
        logger.error("[STAFF DELETE API] Database delete failed", {
      ...
      if (!data || data.length === 0) {
        logger.warn("[STAFF DELETE API] No rows deleted (staff member not found)");
        return apiErrors.notFound("Staff member not found");
      }

      logger.info("[STAFF DELETE API] Staff member deleted successfully", {
        staffId: id,
      });
