...
export async function GET(req: NextRequest) {
  try {
    ...
    const msg =
      e instanceof Error ? getErrorMessage(e) : 'Unknown server error';
...
async function createKDSTickets(supabase: any, order: any) {
  try {
    ...
    if (!existingStations || existingStations.length === 0) {
      logger.debug(
        '[KDS TICKETS] No stations found, creating default stations for venue:',
        order.venue_id
      );
    ...
    const expoStation =
      existingStations.find((s: any) => s.station_type === 'expo') ||
      existingStations[0];
    ...
    for (const item of items) {
      const ticketData = {
        ...
        table_label:
          order.table_id || order.table_number?.toString() || 'Unknown',
        status: 'new',
      };
      ...
      if (ticketError) {
        logger.error(
          '[KDS TICKETS] Failed to create ticket for item:',
          item,
          ticketError
        );
        throw ticketError;
    ...
    logger.debug(
      '[KDS TICKETS] Successfully created',
      items.length,
      'KDS tickets for order:',
      order.id
    );
  } catch (error: unknown) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    logger.debug(
      '[ORDER CREATION DEBUG] Request body:',
    ...
    const table_number =
      tn === null || tn === undefined ? null : Number.isFinite(tn) ? tn : null;
    ...
    if (body.table_number) {
      ...
      if (lookupError) {
        return bad(
          `Failed to check existing tables: ${lookupError.message}`,
          500
        );
      }
        ...
        if (tableCreateErr) {
          ...
          if (tableCreateErr.code === '23505') {
            ...
            if (existingTableAfterError) {
              ...
              return bad(
                `Failed to create table: ${tableCreateErr.message}`,
                500
              );
            }
          } else {
    ...
    const safeItems = body.items.map((it) => ({
      ...
      specialInstructions:
        (it as any).special_instructions ?? it.specialInstructions ?? null,
    }));
    ...
    if (existingOrder) {
      logger.debug(
        '[ORDER API] Found duplicate order, returning existing:',
        existingOrder.id
      );
      return NextResponse.json({
        ok: true,
    ...
    logger.debug(
      '[ORDER CREATION DEBUG] Payment status:',
      payload.payment_status
    );
    logger.debug(
      '[ORDER CREATION DEBUG] Payment method:',
    ...
    logger.debug(
      '[ORDER CREATION DEBUG] Items count:',
    ...
    if (!inserted || inserted.length === 0 || !inserted[0]) {
      ...
      if (fetchError || !fetchedOrder) {
        return bad(
          'Order creation failed: No order data returned from database',
          500
        );
      }
    ...
    const response = {
      ...
      display_name:
        orderSource === 'counter'
          ? `Counter ${table_number}`
          : `Table ${table_number}`, // Include display name for UI
    };

    logger.debug('[ORDER CREATION DEBUG] ===== RETURNING RESPONSE =====');
    logger.debug(
      '[ORDER CREATION DEBUG] Response data:',
    ...
    try {
      ...
      logger.warn(
        '[ORDER CREATION DEBUG] KDS ticket creation failed (non-critical):',
        kdsError
      );
    ...
    const msg =
      e?.message || (typeof e === 'string' ? e : 'Unknown server error');
