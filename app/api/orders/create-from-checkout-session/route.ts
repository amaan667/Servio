...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    let items: Array<{ menu_item_id: string | null; item_name: string; quantity: number; price: number; special_instructions?: string | null }> = [];
    try {
      const raw = meta.items ? JSON.parse(meta.items) : [];
      items = Array.isArray(raw)
        ? raw.map((item: { menu_item_id?: string; id?: string; name?: string; item_name?: string; quantity?: number; price?: number; special_instructions?: string }) => ({
    ...
    if (items.length === 0) {
      items = [{ menu_item_id: null, item_name: "Order", quantity: 1, price: amount, special_instructions: null }];
    }
