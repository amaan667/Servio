...
export async function GET(req: NextRequest) {
  try {
    ...
    const { data: counterStatus, error } = await supabase
      .rpc('get_counter_status', { p_venue_id: venueId });

    if (error) {
      logger.error('[POS COUNTER SESSIONS] Error:', { error: error instanceof Error ? error.message : 'Unknown error' });
      return NextResponse.json({ error: error.message }, { status: 500 });
    }

    return NextResponse.json({ counters: counterStatus });
  } catch (error: unknown) {
    logger.error('[POS COUNTER SESSIONS] Unexpected error:', { error: error instanceof Error ? getErrorMessage(error) : 'Unknown error' });
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!venue_id || !counter_id || !action) {
      return NextResponse.json({ error: 'venue_id, counter_id, and action are required' }, { status: 400 });
    }
