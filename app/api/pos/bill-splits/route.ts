...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (action) {
      case 'create_splits':
        ...
        for (let i = 0; i < splits.length; i++) {
          ...
          if (splitError) {
            logError('[POS BILL SPLITS] Error creating split:', splitError);
          ...
          if (split.order_ids && split.order_ids.length > 0) {
            ...
            if (linksError) {
              logError('[POS BILL SPLITS] Error linking orders:', linksError);
      ...
      case 'pay_split':
        ...
        if (payError) {
          logError('[POS BILL SPLITS] Error paying split:', payError);
    ...
    logError('[POS BILL SPLITS] Unexpected error:', error);
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      logError('[POS BILL SPLITS] Error fetching splits:', error);
