...
export async function handleStartPreparing(supabase: SupabaseClient, table_id: string, order_id: string) {
...
export async function handleMarkReady(supabase: SupabaseClient, table_id: string, order_id: string) {
...
export async function handleMarkServed(supabase: SupabaseClient, table_id: string, order_id: string) {
...
export async function handleCloseTable(supabase: SupabaseClient, table_id: string) {
  try {
    ...
    if (tableError || !table) {
      logger.error('[TABLE ACTIONS] Error fetching table for close:', { error: tableError instanceof Error ? tableError.message : 'Unknown error' });
...
export async function handleReserveTable(supabase: SupabaseClient, table_id: string, customer_name: string, reservation_time: string, reservation_duration: number = 60) {
  try {
    ...
    if (tableError || !table) {
      logger.error('[TABLE ACTIONS] Error fetching table for reservation:', { error: tableError instanceof Error ? tableError.message : 'Unknown error' });
    ...
    if (existingSession) {
      ...
      } else if (['ORDERING', 'IN_PREP', 'READY', 'SERVED', 'AWAITING_BILL'].includes(existingSession.status)) {
        return NextResponse.json({ 
...
export async function handleOccupyTable(supabase: SupabaseClient, table_id: string) {
  ...
  if (existingSession) {
    ...
    if (tableError) {
      logger.error('[TABLE ACTIONS] Error getting table info:', { error: tableError instanceof Error ? tableError.message : 'Unknown error' });
      if (tableError.code === 'PGRST116') {
        const { data: allTables } = await supabase
          .from('tables')
          .select('id, label, venue_id')
...
export async function handleMoveTable(supabase: SupabaseClient, table_id: string, destination_table_id: string) {
  // Get current session
...
export async function handleMergeTable(supabase: SupabaseClient, venue_id: string, table_id: string, destination_table_id: string) {
  try {
    ...
    if (error) {
      logger.error('[TABLE ACTIONS] Error merging tables:', { error: error instanceof Error ? error.message : 'Unknown error' });
...
export async function handleUnmergeTable(supabase: any, table_id: string) {
  try {
    ...
    if (currentTable.merged_with_table_id) {
      ...
      if (error) {
        logger.error('[TABLE ACTIONS] Error unmerging table:', { error: error instanceof Error ? error.message : 'Unknown error' });
