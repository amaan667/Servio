...
export async function handleOccupyTable(supabase: SupabaseClient, table_id: string) {
  logger.info("[OCCUPY TABLE] Starting", { tableId: table_id });
  ...
  logger.debug("[OCCUPY TABLE] Session check", {
    existingSession,
  ...
  logger.debug("[OCCUPY TABLE] Table lookup", {
    table,
  ...
  if (existingSession) {
    logger.info("[OCCUPY TABLE] Updating existing session to OCCUPIED");
    ...
    logger.debug("[OCCUPY TABLE] Update result", {
    ...
    logger.info("[OCCUPY TABLE] Creating new OCCUPIED session");
    ...
    logger.debug("[OCCUPY TABLE] Create result", {
  ...
  logger.info("[OCCUPY TABLE] SUCCESS - Returning success response", { tableId: table_id });
  return NextResponse.json({ success: true });
}
