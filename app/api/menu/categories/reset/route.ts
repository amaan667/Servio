...
export async function POST(request: NextRequest) {
  try {
    ...
    if (manuallyAddedCategories.length > 0) {
      ...
      if (deleteError) {
        logger.error('[CATEGORIES RESET] Error deleting items from manual categories:', deleteError);
        return NextResponse.json({ 
          ok: false, 
    ...
    logger.error('[CATEGORIES RESET] Error in reset categories API:', { error: error instanceof Error ? getErrorMessage(error) : 'Unknown error' });
    return NextResponse.json({ 
      ok: false, 
