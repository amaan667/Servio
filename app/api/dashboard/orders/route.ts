...
export async function GET(req: Request) {
  ...
  if (error) {
    logError('[DASHBOARD ORDERS] Error:', error);
  ...
  logInfo('[DASHBOARD_ORDERS] ===== DASHBOARD ORDERS DEBUG =====');
  logInfo('[DASHBOARD_ORDERS] Scope:', scope.toUpperCase());
  logInfo('[DASHBOARD_ORDERS] Venue ID:', venueId);
  logInfo('[DASHBOARD_ORDERS] Status Filter:', status);
  logInfo('[DASHBOARD_ORDERS] Limit:', limit);
  logInfo('[DASHBOARD_ORDERS] Order Count:', orders?.length || 0);
  
  if (orders && orders.length > 0) {
    logInfo('[DASHBOARD_ORDERS] Sample Orders (first 3):');
    ...
    logInfo('[DASHBOARD_ORDERS] Age Distribution:', ageDistribution);
    ...
    logInfo('[DASHBOARD_ORDERS] Status Distribution:', statusDistribution);
  } else {
    logInfo('[DASHBOARD_ORDERS] No orders found for this scope');
  }
  logInfo('[DASHBOARD_ORDERS] ===== END DASHBOARD ORDERS DEBUG =====');
