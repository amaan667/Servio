 ...
 */
export const PATCH = withUnifiedAuth(async (req: NextRequest, context, routeParams) => {
 ...
 */
export const DELETE = withUnifiedAuth(async (_req: NextRequest, context, routeParams) => {
  try {
    const { id } = (await routeParams?.params) || {};
