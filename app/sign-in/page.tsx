...
import { signInUser, signInWithGoogle, supabase } from "@/lib/supabase";
  ...
  useEffect(() => {
    const checkExistingSession = async () => {
      try {
        ...
        if (session) {
          logger.info("SIGNIN_PAGE: User already authenticated, redirecting to dashboard");
          router.replace("/dashboard");
          return;
        }
      } catch (error) {
        logger.error("SIGNIN_PAGE: Error checking existing session", { error });
      } finally {
