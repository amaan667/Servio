...
test.describe('Complete Order Flow', () => {
  ...
  test('order appears in dashboard live orders', async ({ page, context }) => {
    ...
    await expect(dashboardPage.locator('[data-testid="order-card"]')).toBeVisible({ timeout: 5000 });
  });
});
...
test.describe('KDS Workflow', () => {
  test('kitchen staff can manage tickets', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="kds-ticket"]').first()).toBeVisible({ timeout: 10000 });
...
test.describe('Real-time Updates', () => {
  test('order updates appear in real-time', async ({ page, context }) => {
    ...
    await expect(dashboardPage.locator('[data-testid="new-order-notification"]')).toBeVisible({ timeout: 5000 });
  });
});
