"use client";

import Link from "next/link";

interface TierRestrictionBannerProps {

}

export function TierRestrictionBanner({
  currentTier,
  requiredTier,
  featureName,
  venueId: _venueId, // Keep for API compatibility but not used in link
  reason,
}: TierRestrictionBannerProps) {
  const tierNames: Record<string, string> = {

  };

  return (
    <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 rounded-lg mt-8">
      <div className="flex">
        <div className="flex-shrink-0">
          <svg className="h-5 w-5 text-yellow-400" viewBox="0 0 20 20" fill="currentColor">
            <path
              fillRule="evenodd"
              d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
              clipRule="evenodd"
            />
          </svg>
        </div>
        <div className="ml-3 flex-1">
          <h3 className="text-sm font-medium text-yellow-800">
            {featureName} requires {tierNames[requiredTier] || requiredTier} tier
          </h3>
          <p className="mt-1 text-sm text-yellow-700">
            {reason ||
              `You're currently on the ${tierNames[currentTier] || currentTier} tier. Upgrade to access this feature.`}
          </p>
          <div className="mt-3">
            <Link
              href={`/select-plan?change=true`}
              className="text-sm font-medium text-yellow-800 underline hover:text-yellow-900"
            >
              Upgrade to {tierNames[requiredTier] || requiredTier} →
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}
