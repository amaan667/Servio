...
export function useMenuItemActions(venueId: string, _onRefresh: () => void) {
  ...
  const handleClearMenu = async () => {
    if (!window.confirm("Are you sure you want to clear the entire menu? This cannot be undone."))
      return;
    ...
    try {
      ...
      const { error } = await supabase.from("menu_items").delete().eq("venue_id", venueUuid);
