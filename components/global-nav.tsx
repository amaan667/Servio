...
export default function GlobalNav() {
  ...
  return (
    <nav
      className={`bg-white border-b border-gray-200 sticky top-0 z-50 w-full ${isHome ? "h-40" : "h-20"}`}
    >
      <div
        className={`max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between ${isHome ? "h-40" : "h-20"}`}
      >
        <Link href="/" className="flex items-center">
          <Image
            src="/assets/servio-logo-updated.png"
            alt="Servio Logo"
            width={isHome ? 320 : 160}
            height={isHome ? 80 : 48}
            className={isHome ? "h-20 w-auto transition-all duration-200" : "h-12 w-auto transition-all duration-200"}
            priority
          />
        </Link>
        {/* Desktop nav */}
        <div className="hidden md:flex items-center space-x-6">
          ...
          {session ? (
            ...
            <>
              <Link
                href="/sign-in"
                className="text-gray-700 hover:text-servio-purple font-medium px-2 py-1 transition-colors"
              >
                Sign In
              </Link>
