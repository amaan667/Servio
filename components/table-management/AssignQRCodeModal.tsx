...
export function AssignQRCodeModal({
  ...
  const fetchQRData = async () => {
    try {
      ...
      const qrUrl = buildOrderUrl(venueId, tableLabel, 'table', qrVersion);
      ...
      handleQRError(err, 'fetch_qr_data');
  ...
  const handlePrint = () => {
    ...
    // Create a print-friendly version with actual QR code
    const printWindow = window.open('', '_blank');
    if (printWindow) {
      const qrCodeUrl = generateQRCodeUrl(qrData.qr_url, 200);
        ...
        <html>
          <head>
            ...
            <style>
              ...
              .qr-code { width: 200px; height: 200px; border: 2px solid #000; margin: 0 auto; }
          ...
          <body>
            ...
            <div class="qr-container">
              <div class="qr-code">
                <img src="${qrCodeUrl}" alt="QR Code for Table ${qrData.table_label}" onload="window.focus(); window.print();" />
              </div>
            </div>
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      handleQRError(err, 'regenerate_qr');
