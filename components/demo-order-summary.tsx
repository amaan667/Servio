...
interface DemoOrderSummaryProps {
  checkoutData: {
    venueId: string;
    venueName?: string;
    tableNumber: number;
...
export default function DemoOrderSummary({ checkoutData, paymentMethod, onClose }: DemoOrderSummaryProps) {
  ...
  const getPaymentSuccessMessage = (paymentMethod: string) => {
  ...
  const getTimelineItems = (): OrderTimelineItem[] => {
    ...
    const currentStatus = 'PLACED'; // Demo always starts at PLACED
  ...
  const paymentMessage = getPaymentSuccessMessage(paymentMethod);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-50 to-blue-50 p-4">
      <div className="max-w-2xl mx-auto">
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <p className="font-semibold">{checkoutData.customerName || 'Demo Customer'}</p>
              </div>
              {checkoutData.customerPhone && (
                <div>
                  <p className="text-sm text-gray-900">Phone Number</p>
                  <p className="font-semibold">{checkoutData.customerPhone}</p>
                </div>
              )}
              <div>
                <p className="text-sm text-gray-900">Order Number</p>
                <p className="font-semibold">#{getShortOrderNumber(demoOrderId)}</p>
              </div>
              <div>
                <p className="text-sm text-gray-900">Table</p>
                <p className="font-semibold">{checkoutData.tableNumber || 'N/A'}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        {/* Order Items */}
        {checkoutData.cart && checkoutData.cart.length > 0 && (
          <Card className="mb-6">
            <CardHeader>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 bg-gray-50 rounded-lg flex items-center justify-center overflow-hidden border border-gray-200">
                          {getItemImagePath(item.name) ? (
                            <img
                              src={`/images/menu/${getItemImagePath(item.name)}`}
                              alt={item.name}
                          ...
                          <div 
                            ...
                            style={{ display: getItemImagePath(item.name) ? 'none' : 'flex' }}
                          >
        ...
        <div className="flex gap-4">
          <Button 
            onClick={() => window.location.href = '/order?venue=demo-cafe&table=1'}
            variant="outline"
            className="flex-1"
          >
            Place Another Order
