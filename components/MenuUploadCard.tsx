...
export function MenuUploadCard({ venueId, onSuccess }: MenuUploadCardProps) {
  ...
  const processFile = async (file: File) => {
    ...
    try {
      logInfo('[MENU UPLOAD] Starting PDF upload for venue:', venueId, 'mode:', isReplacing ? 'replace' : 'append');
      ...
      if (fileExtension === '.pdf') {
        if (isReplacing) {
          ...
          logInfo('[MENU UPLOAD] Using catalog replace endpoint');
          ...
          logInfo('[MENU UPLOAD] Using legacy append endpoint');
  ...
  const handleClearCatalog = async () => {
    ...
    try {
      logInfo('[CLEAR CATALOG] Clearing catalog for venue:', venueId);
