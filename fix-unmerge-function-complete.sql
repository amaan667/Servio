-- Create a complete unmerge function that restores tables to their original state
-- This function will undo a merge and restore both tables as they were before

-- First, drop any existing unmerge function
...
BEGIN
  ...
  -- Expected format: "Table A merged with Table B"
  IF v_merged_table_info.label LIKE '% merged with %' THEN
    -- Split the label to get original table names
  ...
  ELSE
    ...
    v_original_label_a := 'Table ' || EXTRACT_NUMBER(v_merged_table_info.label);
    v_original_label_b := 'Table ' || (EXTRACT_NUMBER(v_merged_table_info.label) + 1);
  END IF;
  
  -- Calculate original seat counts (assume equal distribution or use stored values)
  -- For now, we'll use a default of 2 seats each, but this could be improved
...
-- Helper function to extract numbers from text (for fallback label parsing)
...
BEGIN
  RETURN COALESCE(
    (SELECT (regexp_matches(input_text, '\d+'))[1]::INTEGER),
    1
  );
END;
$$;
