...
import type { RealtimeTable, RealtimeTableSession, PostgresPayload, ConnectionState, SubscriptionStatus } from "@/lib/realtime/types";
...
export function useRealtimeTables(options: UseRealtimeTablesOptions): UseRealtimeTablesReturn {
  const { venueId, includeSessions = true, onTableChange, onSessionChange, enabled = true } = options;
