-- Drop the old UUID version of api_merge_tables function
-- This fixes the "Could not choose the best candidate function" error

-- Drop the old UUID version
DROP FUNCTION IF EXISTS public.api_merge_tables(UUID, UUID, UUID);

-- Ensure the TEXT version exists (recreate it to be sure)
...
GRANT EXECUTE ON FUNCTION api_merge_tables(TEXT, TEXT, TEXT) TO anon, authenticated;
