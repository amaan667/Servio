...
for (const file of files) {
  try {
    ...
    if (content.includes('const auth = await requirePageAuth(') && !content.includes('.catch(() => null)')) {
    ...
    if (content.includes('auth.hasFeatureAccess') && !content.includes('auth?.hasFeatureAccess')) {
      ...
      content = content.replace(/auth\?\.hasFeatureAccess\(([^)]+)\)/g, '(auth?.hasFeatureAccess($1) ?? false)');
      changed = true;
    }
