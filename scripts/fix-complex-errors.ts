...
function fixFile(filePath: string): { fixed: boolean; fixes: string[] } {
  ...
  const hasApiErrors =
    content.includes("apiErrors") || content.includes("from '@/lib/api/standard-response'");
  ...
  const patterns = [
    ...
    {
      regex:
        /return\s+NextResponse\.json\(\s*\{\s*error:\s*['"]([^'"]+)['"],?\s*message:\s*['"]([^'"]+)['"]?\s*\}\s*,\s*\{\s*status:\s*400\s*\}\)/gs,
    ...
    // Pattern: NextResponse.json({ ok: false, error: "..." }, { status: 500 })
    {
      ...
      replacement: (match: string, error: string, status: string) => {
        ...
        if (statusNum === 400)
          return `return apiErrors.badRequest('${error.replace(/'/g, "\\'")}')`;
        if (statusNum === 401)
          return `return apiErrors.unauthorized('${error.replace(/'/g, "\\'")}')`;
    ...
    {
      regex:
        /return\s+NextResponse\.json\(\s*\{\s*error:\s*error\.message\s*\}\s*,\s*\{\s*status:\s*500\s*\}\)/g,
      replacement: () => {
