-- Fix the api_merge_tables function to properly handle session_id
-- The issue is that the function tries to insert into table_session_links without providing session_id

-- Drop the existing function
DROP FUNCTION IF EXISTS public.api_merge_tables(TEXT, UUID, UUID);

-- Create the corrected version that properly handles session_id
...
BEGIN
  ...
  -- This uses the original schema with session_id
  INSERT INTO table_session_links (session_id, table_id, created_at)
  VALUES (v_primary_session, v_secondary_table, NOW())
