-- Fix dashboard_counts function to only count paid orders
-- This ensures revenue and order counts only include successfully paid orders
...
-- normalize today's PAID orders only (avoid inconsistent filters)
today as (
  select
    o.id,
    o.table_number,
  ...
  where o.venue_id = p_venue_id
    and o.payment_status = 'PAID'  -- Only count paid orders
...
hist as (
  -- All PAID orders from previous days (not just SERVED)
  select count(*)::int c
  from public.orders o, b
  where o.venue_id = p_venue_id
    and o.payment_status = 'PAID'  -- Only count paid orders
...
-- Test the function
SELECT 'Dashboard counts function updated to only count paid orders' as status;
