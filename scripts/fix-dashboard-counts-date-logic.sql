-- Fix dashboard_counts function to properly handle date boundaries
-- The issue: Orders from yesterday were appearing in "Today" tab because
-- the function was only checking if orders were within 30 minutes, not if they were from today
...
live as (
  ...
  where t.status in ('PLACED','ACCEPTED','IN_PREP','READY','SERVING','SERVED','COMPLETED')
    ...
    -- CRITICAL FIX: Ensure live orders are actually from today
...
earlier as (
  ...
  -- CRITICAL FIX: Earlier today should only include orders from today that are not in live
  and t.created_at >= b.start_utc
),
hist as (
...
active_tables as (
  ...
  where t.table_number is not null
    ...
    and t.created_at >= b.now_utc - make_interval(mins => p_live_window_mins)
    -- CRITICAL FIX: Ensure active tables are from today
    and t.created_at >= b.start_utc
)
select
  live.c,
