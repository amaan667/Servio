...
## Problem
The database schema needs to be updated to support the new order status system. The error "Could not find the 'order_status' column" indicates the database hasn't been updated yet.
...
### Step 2: Update payment_status constraints
```sql
...
### Step 4: Drop old column and create indexes
```sql
...
3. Run each step above in order
4. Test by creating a new order

## Expected Results:
- New orders will have `order_status` and `payment_status` fields
- Existing orders will be migrated to the new status system
- The "Insert failed" error should be resolved
- Active tables count will be accurate (only tables with non-completed orders)
- Payment status will show correctly (PAID/UNPAID)
