-- Fix the merge function to properly handle existing sessions
...
DROP FUNCTION IF EXISTS api_merge_tables(p_venue_id TEXT, p_table_a TEXT, p_table_b TEXT);
DROP FUNCTION IF EXISTS api_merge_tables(p_venue_id TEXT, p_table_a UUID, p_table_b UUID);

-- Create a single, properly typed function that handles sessions correctly
...
BEGIN
  ...
  BEGIN
    v_table_a_uuid := p_table_a::UUID;
    v_table_b_uuid := p_table_b::UUID;
  EXCEPTION
    WHEN invalid_text_representation THEN
      RAISE EXCEPTION 'Invalid UUID format for table IDs: % or %', p_table_a, p_table_b;
  END;
  
  -- Get table information for both tables (using UUID comparison)
  SELECT *
  INTO v_table_a_info
  FROM tables 
  WHERE id = v_table_a_uuid AND venue_id = p_venue_id;
  
  SELECT *
  INTO v_table_b_info
  FROM tables 
  WHERE id = v_table_b_uuid AND venue_id = p_venue_id;
  ...
  -- Get current sessions for both tables (using UUID comparison)
  SELECT *
  INTO v_session_a
  FROM table_sessions 
  WHERE table_id = v_table_a_uuid AND closed_at IS NULL;
  
  SELECT *
  INTO v_session_b
  FROM table_sessions 
  WHERE table_id = v_table_b_uuid AND closed_at IS NULL;
  ...
  -- Create merged label (combine the labels with " merged with " format)
  ...
  SET 
    ...
    updated_at = NOW()
  WHERE id = v_table_a_uuid;
  
  -- Mark table B as merged with table A
  UPDATE tables 
  SET 
    merged_with_table_id = v_table_a_uuid,
    updated_at = NOW()
  WHERE id = v_table_b_uuid;
  
  -- Handle session for table A properly
  IF v_session_a IS NOT NULL THEN
    -- Update existing session to FREE (don't create a new one)
    UPDATE table_sessions 
    SET 
      status = 'FREE',
      updated_at = NOW()
    WHERE table_id = v_table_a_uuid AND closed_at IS NULL;
  ELSE
    -- Only create new FREE session if none exists
    INSERT INTO table_sessions (table_id, venue_id, status, opened_at, created_at, updated_at)
    VALUES (v_table_a_uuid, p_venue_id, 'FREE', NOW(), NOW(), NOW())
