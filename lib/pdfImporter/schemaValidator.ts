  ...
  try {
    ...
    for (const category of validated.categories) {
      for (const item of category.items) {
        ...
        if (item.price > 100) {
          warnings.push(
            `Item "${item.title}" has unusually high price: £${item.price}`
          );
        }
        ...
        const duplicateTitles = category.items.filter(
          (i) => i.title.toLowerCase() === item.title.toLowerCase()
        );
        if (duplicateTitles.length > 1) {
          errors.push(
            `Duplicate item title "${item.title}" in category "${category.name}"`
          );
        }
        ...
        if (item.variants) {
          for (const variant of item.variants) {
            if (variant.price <= 0) {
              errors.push(
                `Variant "${variant.name}" of "${item.title}" has invalid price: ${variant.price}`
              );
            }
            if (variant.priceAdd < 0) {
              warnings.push(
                `Variant "${variant.name}" of "${item.title}" has negative price add: ${variant.priceAdd}`
              );
            }
          }
        }
        ...
        if (item.options) {
          for (const option of item.options) {
            if (option.choices.length === 0) {
              errors.push(
                `Option group "${option.group}" for "${item.title}" has no choices`
              );
            }
            ...
            for (const choice of option.choices) {
              if (choice.priceAdd < 0) {
                warnings.push(
                  `Option choice "${choice.name}" for "${item.title}" has negative price add: ${choice.priceAdd}`
                );
              }
            }
          }
    ...
    const emptyCategories = validated.categories.filter(
      (cat) => cat.items.length === 0
    );
    if (emptyCategories.length > 0) {
      warnings.push(
        `Empty categories found: ${emptyCategories.map((c) => c.name).join(', ')}`
      );
    }
    ...
    if (itemsCount > 100) {
      warnings.push(
        `Large number of items (${itemsCount}) - check for modifier explosion`
      );
    }
    ...
    if (validated.categories.length > 20) {
      warnings.push(
        `Large number of categories (${validated.categories.length}) - check for over-segmentation`
      );
    }
  } catch (error: unknown) {
...
export async function replaceCatalogAtomically(
  ...
  try {
    ...
    logger.error(
      '[CATALOG_REPLACE] Atomic replacement failed:',
...
export async function validateCatalogPayload(
  ...
  try {
    ...
    logger.error(
      '[CATALOG_VALIDATE] Validation failed:',
      getErrorDetails(error)
    );
    return {
      valid: false,
 ...
 */
export function enforceCategoryGuards(
  categories: ParsedCategory[]
): ParsedCategory[] {
  ...
  return categories.map((category) => {
    ...
    const filteredItems = category.items.filter((item) => {
      ...
      const isAllowed = allowedItems.some((allowed) =>
        itemTitle.includes(allowed)
      );
