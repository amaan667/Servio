...
export function storePkceVerifier(verifier) {
  try {
    ...
    console.log('[OAuth Frontend] Stored PKCE verifier in sessionStorage', {
...
export function getPkceVerifier() {
  try {
    ...
    console.log('[OAuth Frontend] Retrieved PKCE verifier from sessionStorage', {
      hasVerifier: !!verifier,
      verifierLength: verifier?.length,
      timestamp: new Date().toISOString()
    });
...
export function clearPkceVerifier() {
  try {
    ...
    console.log('[OAuth Frontend] Cleared PKCE verifier from sessionStorage');
  } catch (error) {
