...
export async function getAuthenticatedUser() {
  try {
    ...
    if (error) {
      logError('[AUTH CLIENT] Error getting authenticated user:', error);
      return { user: null, error: error.message };
    }
    
    return { user, error: null };
  } catch (error) {
    logError('[AUTH CLIENT] Exception getting authenticated user:', error);
...
export async function signOut() {
  try {
    ...
    if (error) {
      logError('[AUTH CLIENT] Error signing out:', error);
      return { success: false, error: error.message };
    }
    
    return { success: true, error: null };
  } catch (error) {
    logError('[AUTH CLIENT] Exception signing out:', error);
