...
export function mapOrderToCardData(legacyOrder: LegacyOrder, currency: string = 'GBP'): OrderForCard {
  ...
  const generateLabels = (order: LegacyOrder): { 
    ...
    // Debug logging
    logInfo(`[mapOrderToCardData DEBUG] Order ${order.id}:`, {
      source: order.source,
    ...
    if (order.table_number) {
      logInfo(`[mapOrderToCardData DEBUG] Order ${order.id} has table_number: ${order.table_number}`);
    } else {
      logInfo(`[mapOrderToCardData DEBUG] Order ${order.id} has NO table_number`);
    }
    
    if (isCounterOrder) {
      logInfo(`[mapOrderToCardData DEBUG] Order ${order.id} classified as COUNTER order`);
      ...
      logInfo(`[mapOrderToCardData DEBUG] Order ${order.id} classified as TABLE order`);
      ...
      if (order.table_label) {
        ...
        logInfo(`[mapOrderToCardData DEBUG] Using existing table_label: ${generatedTableLabel}`);
      } else if (order.table_number) {
        generatedTableLabel = `Table ${order.table_number}`;
        logInfo(`[mapOrderToCardData DEBUG] Generated table_label from table_number: ${generatedTableLabel}`);
      } else {
        // Fallback for orders without table numbers - use a generic label
        generatedTableLabel = 'Table Order';
        logInfo(`[mapOrderToCardData DEBUG] Using fallback table_label: ${generatedTableLabel}`);
      }
      logInfo(`[mapOrderToCardData DEBUG] Final generated table_label: ${generatedTableLabel}`);
      return {
  ...
  logInfo(`[mapOrderToCardData DEBUG] Final result for order ${legacyOrder.id}:`, {
    table_label,
    counter_label,
  ...
  logInfo(`[mapOrderToCardData DEBUG] Final table_label value: "${table_label}"`);
  logInfo(`[mapOrderToCardData DEBUG] Final counter_label value: "${counter_label}"`);
