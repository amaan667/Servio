...
export async function executeMenuUpdatePrices(
  ...
  logger.debug(
    `[AI ASSISTANT] Updating prices for ${params.items.length} items`
  );

  // Validate that we have items to update
  if (!params.items || params.items.length === 0) {
    throw new AIAssistantError(
      'No items specified for price update',
      'INVALID_PARAMS'
    );
  }
  ...
  if (!currentItems || currentItems.length === 0) {
    throw new AIAssistantError(
      'No items found matching the provided IDs',
      'INVALID_PARAMS'
    );
  }
  ...
  const maxChangePercent =
    DEFAULT_GUARDRAILS['menu.update_prices'].maxPriceChangePercent || 20;
  ...
  for (const item of params.items) {
    ...
    const changePercent = Math.abs(
      ((item.newPrice - current.price) / current.price) * 100
    );
    logger.debug(
  ...
  // Preview mode
  if (preview) {
    const before = currentItems.map((i) => ({
      id: i.id,
      name: i.name,
      price: i.price,
    }));
  ...
  logger.debug(
    `[AI ASSISTANT] Executing price updates for ${params.items.length} items`
  );
  let updatedCount = 0;
  ...
  for (const item of params.items) {
    ...
    if (error) {
      logger.error(
        `[AI ASSISTANT] Failed to update price for "${itemName}":`,
        error
      );
      failedUpdates.push({ id: item.id, name: itemName, error: error.message });
    } else if (!data || data.length === 0) {
      logger.error(
        `[AI ASSISTANT] No item updated for "${itemName}" - possibly wrong venue_id`
      );
      failedUpdates.push({
        id: item.id,
        name: itemName,
      ...
      logger.debug(
        `[AI ASSISTANT] Successfully updated "${itemName}" to £${item.newPrice}`
      );
      updatedCount++;
    }
  }
...
export async function executeMenuTranslate(
  ...
  logger.debug(
    `[AI ASSISTANT] Starting translation of ${items.length} items to ${params.targetLanguage}`
  );
  ...
  const targetLangName =
    languageNames[params.targetLanguage] || params.targetLanguage;
  ...
  const detectSourceLanguage = (
    ...
    logger.debug(
      `[AI ASSISTANT] Language detection: ${spanishCount} Spanish indicators, ${englishCount} English indicators`
    );
    ...
    if (spanishCount > englishCount) {
      ...
      logger.debug(
        `[AI ASSISTANT] Ambiguous language, defaulting to opposite of target: ${defaultSource}`
      );
  ...
  if (preview) {
    ...
    try {
      ...
      const itemsToTranslate = sampleItems.map((item) => ({
        ...
        ...(params.includeDescriptions && item.description
...
Remember: You MUST return ALL ${sampleItems.length} items with translations. No exceptions.`;
    ...
    return {
      ...
      after: items.slice(0, 5).map((i) => ({
        ...
        description: i.description
          ? `[Will translate to ${targetLangName}] ${i.description}`
          : '',
        category: i.category
          ? `[Will translate to ${targetLangName}] ${i.category}`
          : '',
      })),
      impact: {
        itemsAffected: items.length,
  ...
  try {
    ...
    for (let i = 0; i < items.length; i += batchSize) {
      ...
      logger.debug(
        `[AI ASSISTANT] Processing batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(items.length / batchSize)} (${batch.length} items)`
      );
...
Remember: You MUST return ALL ${batch.length} items with translations. No exceptions.`;
      ...
      while (!batchTranslated && retryCount < maxRetries) {
        try {
          ...
          if (content) {
            logger.debug(
              `[AI ASSISTANT] Translation response for batch ${Math.floor(i / batchSize) + 1}:`,
            ...
            if (translatedArray.length === batch.length) {
              ...
              if (validItems.length === batch.length) {
                logger.debug(
                  `[AI ASSISTANT] Batch ${Math.floor(i / batchSize) + 1} translation successful: ${translatedArray.length} items`
                );
                translatedItems.push(...translatedArray);
                batchTranslated = true;
              } else {
                logger.error(
                  `[AI ASSISTANT] Batch ${Math.floor(i / batchSize) + 1} has ${validItems.length} valid items, expected ${batch.length}`
                );
                retryCount++;
                if (retryCount < maxRetries) {
                  logger.debug(
                    `[AI ASSISTANT] Retrying batch ${Math.floor(i / batchSize) + 1} (attempt ${retryCount + 1})`
                  );
                }
              }
            } else {
              logger.error(
                `[AI ASSISTANT] Batch ${Math.floor(i / batchSize) + 1} returned ${translatedArray.length} items, expected ${batch.length}`
              );
              retryCount++;
              if (retryCount < maxRetries) {
                logger.debug(
            ...
            logger.error(
              `[AI ASSISTANT] No content in translation response for batch ${Math.floor(i / batchSize) + 1}`
            );
            retryCount++;
          }
        } catch (batchError) {
      ...
      if (!batchTranslated) {
        logger.error(
          `[AI ASSISTANT] Batch ${Math.floor(i / batchSize) + 1} failed after ${maxRetries} retries, using original items`
        );
    ...
    if (translatedItems.length !== originalItemCount) {
      logger.error(
        `[AI ASSISTANT] Item count mismatch: expected ${originalItemCount}, got ${translatedItems.length}`
      );
      throw new AIAssistantError(
    ...
    logger.debug(
      `[AI ASSISTANT] Updating ${translatedItems.length} translated items in database`
    );
    let updatedCount = 0;
    let failedCount = 0;
    ...
    for (const translatedItem of translatedItems) {
      ...
      logger.debug(
        `[AI ASSISTANT] Updating item ${translatedItem.id}: ${translatedItem.name}, category: ${translatedItem.category}`
      );
      ...
      if (!error) {
        ...
        logger.error(
          `[AI ASSISTANT] Failed to update item ${(translatedItem as any).id}:`,
          error
        );
        failedCount++;
      }
    ...
    logger.error('[AI ASSISTANT] Translation error:', {
      error: error instanceof Error ? getErrorMessage(error) : 'Unknown error',
    });
    throw new AIAssistantError(
      `Translation failed: ${error instanceof Error ? getErrorMessage(error) : 'Unknown error'}`,
      'EXECUTION_FAILED',
      error
    );
  }
